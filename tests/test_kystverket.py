...
class TestReceive:
    ...
    def test_msg_3_postion_report(self):
        raw = ['\\s:2573535,c:1601581333*05\\!BSVDM,1,1,,A,33mGpL5000QE5URWuPIIj9LL0Dg:,0*02\r']
    ...
    def test_msg_1_postion_report(self):
        raw = ['\\s:2573515,c:1601654238*06\\!BSVDM,1,1,,A,13mdit?P00Q>HhRWL76v4?vT20S?,0*70\r']
        msg = kystverket.read_raw(raw)
        print(msg)        
        assert msg['id'] == 1
...
class TestProblemMessages:
    def test_bad_bit_count(self):
        raw = ["\s:2573575,c:1601669415*07\!BSVDM,1,1,,B,H3mfmU0l4LqE<00000000000000,2*5A"]
        msg = kystverket.read_raw(raw)
        assert msg['id'] == 3
        assert msg['mmsi'] == 1
        assert 0 == 1
